...
export default function WeightTracker({
    ...
    const [range, setRange] =
        useState<NonNullable<WeightTrackerProps['initialRange']>>(initialRange);
    ...
    const filtered = useMemo(() => {
        ...
        if (days === Number.POSITIVE_INFINITY) return localData;
    ...
    async function handleSubmit() {
        ...
        if (isEditingToday && todayEntry) {
            ...
            try {
                ...
                setLocalData((prev) =>
                    prev.map((e) =>
                        e.date === todayKey ? { ...e, weight: numeric, date: todayKey } : e
                    )
                );
            } finally {
                setShowModal(false);
                ...
                <div className="flex items-center gap-2">
                    {!todayEntry && (
                        <button
                            ...
                            className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 hover:opacity-90"
                        >
                    ...
                    {todayEntry && (
                        <button
                            ...
                            className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800 cursor-pointer"
                        >
                        ...
                        >
                            ...
                            <Tooltip
                                ...
                                labelFormatter={(label) =>
                                    parseLocalDate(label).toLocaleDateString()
                                }
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-sm rounded-2xl bg-white dark:bg-zinc-900 shadow-xl border border-zinc-200 dark:border-zinc-800 p-5">
                        <div className="text-lg font-semibold mb-1">
                            {isEditingToday ? "Edit today's weight" : "Add today's weight"}
                        </div>
                        <div className="text-xs text-zinc-500 mb-4">
                            {new Date().toLocaleDateString()} ({unitLabel})
                        </div>
                        ...
                        <div className="mt-5 flex items-center justify-end gap-2">
                            <button
                                onClick={() => setShowModal(false)}
                                className="px-3 py-2 text-sm rounded-xl border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800 cursor-pointer"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={handleSubmit}
                                className="px-3 py-2 text-sm rounded-xl bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 disabled:opacity-50 cursor-pointer"
