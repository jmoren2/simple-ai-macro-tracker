...
export default function Logs({ user, calorieGoal, apiUrl }: Props) {
    ...
    return (
        <div className="min-h-screen bg-brand-bg text-brand-text p-4">
            ...
            >
                <h1 className="text-2xl font-bold text-center mb-6 text-brand-accent">
                    📅 Daily Logs
                </h1>
                        ...
                        return (
                            <div key={date} className="mb-6">
                                <div className="flex justify-between items-center border-b border-brand-muted pb-1 mb-2">
                                    <h2 className="text-md font-semibold text-brand-accent">
                                        {date}
                                    </h2>
                                    <div
                                        className={`text-sm text-brand-muted text-right ${totals.calories > calorieGoal ? 'text-red-500' : 'text-green-600'}`}
                                    >
                                        {totals.calories} cal
                                        <br />
                                        {totals.protein.toFixed(1)}p · {totals.carbs.toFixed(1)}c ·{' '}
                                    ...
                                    {entries.map((log, i) => (
                                        ...
                                        >
                                            ...
                                            <span className="text-xs text-brand-muted text-right whitespace-nowrap flex items-center gap-1">
                                                {log.calories ?? '?'} cal · {log.protein ?? '?'}p ·{' '}
                                                {log.carbs ?? '?'}c · {log.fat ?? '?'}f{' '}
                                                <MdOutlineCancel
